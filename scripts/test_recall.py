...
def prepare_collection(
    ...
    """创建（或加载）Milvus collection，用于向量存储与检索"""
    ...
    schema = CollectionSchema(fields, description="vector recall test")
    coll = Collection(name, schema=schema, using=alias)
    # 为向量字段创建索引
    coll.create_index(
        "emb",
        {
            ...
            "metric_type": metric,
            # "params": {"nlist": 128}
            "params": {"M": 2, "efConstruction": 8}
            # "params": {}
        },
        using=alias
    )
    # 必要时加载到内存
    coll.load(using=alias)
    return coll


def add_vectors_to_milvus(collection: Collection, added_data, batch_size: int = 1024):
    """批量插入向量到 Milvus"""
    total = len(added_data)
    print(f"Inserting {total} vectors into Milvus…")
    pks, vecs = [], []
    for i, row in added_data.iterrows():
        pks.append(int(row.iloc[8]))
        vecs.append([float(x) for x in row.iloc[9].split(':')])
        # 分批插入
        if len(pks) >= batch_size:
            collection.insert([pks, vecs])
            pks.clear();
            vecs.clear()
    if pks:
        collection.insert([pks, vecs])
...
def test_recall_milvus(
    ...
    # 第一步：连接并准备 collection
    connect_milvus(host, port)
    # 维度从 added_data 第一行向量长度推断
    dim = len(added_data.iloc[0, 9].split(':'))
    coll = prepare_collection(collection_name, dim)

    # 插入向量
    add_vectors_to_milvus(coll, added_data)

    total_matches = 0
    total_truth = 0
    num_queries = len(search_data)
    print(f"Running {num_queries} queries, top {k}")

    for i, row in search_data.iterrows():
        # 构造查询向量
        query_vec = [float(x) for x in row.iloc[9].split(':')]

        # 获取地面真值
        exact_neighbors, _ = find_exact_neighbors(row.iloc[9], added_data, k)
        ...
        results = coll.search(
            ...
            # param={"metric_type": "L2", "params": {"ef": 50}},  # efSearch=50
            param={},
            limit=k,
            output_fields=[]
        )
        # 提取返回的 pk
        milvus_pks = [str(hit.id) for hit in results[0]]

        # 计算召回
        hits = set(exact_neighbors) & set(milvus_pks)
        match_count = len(hits)
        total_matches += match_count
        total_truth += len(exact_neighbors)
        recall = match_count / len(exact_neighbors) if exact_neighbors else 0

        print(f"Query {i}: recall {recall:.4f} ({match_count}/{len(exact_neighbors)})")

    overall = total_matches / total_truth if total_truth else 0
    print(f"\nOverall Milvus recall: {overall:.4f} ({total_matches}/{total_truth})")
    return overall
...
if __name__ == '__main__':
    ...
    parser.add_argument('--search_file', type=str, default='./data/search_vectors_data.csv',
                        help='Path to search vectors CSV file')
    ...
    parser.add_argument('--test_usearch', type=bool, default=False,
                        help='Use USearch instead of Redis (default: False)')
    parser.add_argument('--test_eloqvec', type=bool, default=False,
                        help='Use Eloqvec instead of Redis (default: False)')
    parser.add_argument('--test_milvus', type=bool, default=False,
                        help='Use Milvus instead of Redis (default: False)')
    parser.add_argument('--test_correctness', type=bool, default=False,
                        help='Compare the search result of eloqvec and usearch (default: False)')
    parser.add_argument('--mock_test_correctness', type=bool, default=False,
                        help='Compare the search result of eloqvec and usearch (default: False)')
    ...
    if args.test_milvus:
        print(f"Running recall test with {args.test_num} test vectors...")
        test_recall_milvus("172.26.132.32", "19530", "c" + str(int(time.time())), search_data, added_data, args.k)
